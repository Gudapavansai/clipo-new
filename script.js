...
if (heroVideo && heroPlayBtn && heroPoster) {
    let isPlaying = false;

    // Check if video has a valid source
    const hasVideoSource = heroVideo.querySelector('source') !== null ||
        (heroVideo.src && heroVideo.src !== '' && heroVideo.src !== window.location.href);

    heroPlayBtn.addEventListener('click', () => {
        if (hasVideoSource) {
            // If video source exists, play/pause the video
            if (isPlaying) {
                heroVideo.pause();
                ...
                heroVideo.play().then(() => {
                    ...
                    // Fallback: just show a message or do nothing
                });
            }
        } else {
            // If no video source, you can add custom behavior here
            // For example, open a modal with video, redirect to YouTube, etc.
            console.log('No video source available. Add a video file to ./videos/ folder.');

            // Optional: Show an alert or open a modal
    ...
    // Auto-hide poster if video starts playing (e.g., autoplay)
